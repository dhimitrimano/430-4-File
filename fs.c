...
i32 fsRead(i32 fd, i32 numb, void* buf) {
  ...
  i8* fer = (i8 *)buf;
  int ret = 0;
  i32 curr_pos = fsTell(fd);
  i32 end_pos = curr_pos + numb;

  i32 end_fbn = end_pos / BYTESPERBLOCK;
  i32 end_dbn = bfsFbnToDbn(bfsFdToInum(fd), end_fbn);
  i32 h = curr_pos / BYTESPERBLOCK;
  i32 curr_dbn = bfsFbnToDbn(bfsFdToInum(fd), h);

  while (curr_dbn > end_dbn) {
    end_fbn -= 1;
    end_dbn = bfsFbnToDbn(bfsFdToInum(fd), end_fbn);
  }

  do {
    curr_dbn = bfsFbnToDbn(bfsFdToInum(fd), h);
    i8 bio_buf[BYTESPERBLOCK];
    bioRead(curr_dbn, bio_buf);
    i32 i = fsTell(fd) % BYTESPERBLOCK;
    for (; ret < numb && i < BYTESPERBLOCK && fsTell(fd) < fsSize(fd); i += 1, ret += 1) {
      fer[ret] = bio_buf[i];
...
i32 fsWrite(i32 fd, i32 numb, void* buf) {
  ...
  i32 h = curr_pos / BYTESPERBLOCK;
  i32 curr_dbn = bfsFbnToDbn(bfsFdToInum(fd), h);
  i32 pre_expansion_fbn = end_fbn;
  i32 pre_expansion_dbn = end_dbn;

  if (curr_dbn > end_dbn) {
    while (curr_dbn > pre_expansion_dbn) {
      pre_expansion_fbn -= 1;
      pre_expansion_dbn = bfsFbnToDbn(bfsFdToInum(fd), pre_expansion_fbn);
    }
    bfsExtend(bfsFdToInum(fd), end_fbn);
    end_dbn = bfsFbnToDbn(bfsFdToInum(fd), end_fbn);
  }
  ...
  do {
    ...
    i8 bio_buf[BYTESPERBLOCK];
    bioRead(curr_dbn, bio_buf);
    i = fsTell(fd) % BYTESPERBLOCK;
    for (; bio_index < numb && i < BYTESPERBLOCK; i += 1, bio_index += 1) {
      fsSeek(fd, 1, SEEK_CUR);
      if (fsTell(fd) > fsSize(fd)) {
        bfsSetSize(bfsFdToInum(fd), fsSize(fd) + 1);
      }
      bio_buf[i] = fer[bio_index];
    }

    bioWrite(curr_dbn, bio_buf);
    h += 1;
  } while (curr_dbn < end_dbn);
